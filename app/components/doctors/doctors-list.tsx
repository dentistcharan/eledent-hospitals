...
import { fetchDoctors, generateDoctorSlug, type ApiDoctor } from "@/lib/doctors-api";

function DoctorCard({ doctor }: { doctor: ApiDoctor & { slug: string } }) {
            ...
            <div className="mx-auto max-w-7xl px-6 lg:px-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {doctors.map((doctor, idx) => (
                        <DoctorCard
                            key={idx}
                            doctor={{ ...doctor, slug: generateDoctorSlug(doctor.name) }}
                        />
                    ))}
                </div>
